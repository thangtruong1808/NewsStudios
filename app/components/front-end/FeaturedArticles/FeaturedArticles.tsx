...
// Component to display featured articles in a grid layout
export default function FeaturedArticles() {
  ...
  const fetchFeaturedArticles = async (page: number = 1) => {
    try {
      setLoading(true);
      ...
      setHasMore(
        (result.data?.length || 0) + (page - 1) * ITEMS_PER_PAGE <
          (result.totalCount || 0)
      );
      setError(null);
    } catch (err) {
  ...
  useEffect(() => {
    fetchFeaturedArticles(currentPage);
  }, [currentPage]);
  ...
  return (
    <>
      ...
      <div className="space-y-8 mt-8">
        {/* Image Carousel */}
        <ImageCarousel articles={articles.slice(0, 5)} />
